...
func (s *testTxStructureSuite) TestHash(c *C) {
	...
	_, err = tx.HSet(key, []byte("1"), []byte("1"))
	c.Assert(err, IsNil)

	_, err = tx.HSet(key, []byte("2"), []byte("2"))
	c.Assert(err, IsNil)
	...
	_, err = tx.HDel(key, [][]byte{[]byte("fake_key")})
	c.Assert(err, IsNil)
	...
	_, err = tx.HSet(key, []byte("nil_key"), nil)
	c.Assert(err, IsNil)
	...
	_, err = tx.HSet(key, []byte("nil_key"), []byte("1"))
	c.Assert(err, IsNil)
	...
	_, err = tx.HSet(key, []byte("nil_key"), []byte("2"))
	c.Assert(err, IsNil)
	...
	dkey := []byte("hashdel")
	_, err = tx.HSet(dkey, []byte("1"), []byte("1"))
	c.Assert(err, IsNil)

	c, err := tx.DEL([][]byte{dkey})
	c.Assert(err, IsNil)
	c.Assert(value, DeepEquals, []byte("2"))
