...
func RotateDel() error {
	return _log.DoRotateDel()
}
...
func SetKeepAge(t uint) {
	_log.SetRotateMaxAge(time.Duration(t) * time.Hour)
}
...
func (l *logger) SetRotateMaxAge(t time.Duration) {
	l.maxAge = t
}

func (l *logger) DoRotateDel() error {
	return l.doRotateDel()
}
...
func (l *logger) doRotateDel() error {
	...
	logMatch := l.fileName + "." + l.logSuffix[:2] + "[0-9]*"
	matches, err := filepath.Glob(logMatch)
	if err != nil {
		return err
	}
